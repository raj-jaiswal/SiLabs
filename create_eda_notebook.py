...
Programmatically builds dataset_eda.ipynb using nbformat with 5 logical sections
covering cohort statistics, vitals distributions, engineered features, time-series telemetry,
and adverse event risk profiling across 15 high-resolution figures.
"""
...
def create_notebook():
    ...
    cells.append(nbf.v4.new_markdown_cell(
        ...
        "\n"
        "This notebook performs a comprehensive Exploratory Data Analysis (EDA) across high-resolution perioperative patient telemetry records, "
        "raw waveform biosignals, engineered hemodynamic features, multi-parameter time series, and 10-minute forward adverse event target labels."
    ))
    ...
    cells.append(nbf.v4.new_code_cell(fig01_code))
    ...
    cells.append(nbf.v4.new_code_cell(fig02_code))
    ...
    cells.append(nbf.v4.new_markdown_cell(
        "## Section 2: Core Vitals & Signal Quality\n"
        "\n"
        "Exploration of core physiological vitals (HR, SBP, DBP, MBP, SpO2, Resp Rate), joint distributions, "
        "physiological hierarchy compliance (SBP > MBP > DBP), and raw waveform artifact identification."
    ))
    ...
    cells.append(nbf.v4.new_code_cell(fig03_code))
    ...
    cells.append(nbf.v4.new_code_cell(fig04_code))
    ...
    cells.append(nbf.v4.new_code_cell(fig05_code))
    ...
    cells.append(nbf.v4.new_markdown_cell(
        "## Section 3: Feature Engineering & Hemodynamic Dynamics\n"
        "\n"
        "Evaluation of engineered features (Shock Index, Pulse Pressure, Mean Arterial Pressure), correlation matrices, "
        "dynamic rolling statistics (60s window), and distribution ridgelines."
    ))
    ...
    cells.append(nbf.v4.new_code_cell(fig06_code))
    ...
    cells.append(nbf.v4.new_code_cell(fig07_code))
    ...
    cells.append(nbf.v4.new_code_cell(fig08_code))
    ...
    cells.append(nbf.v4.new_code_cell(fig09_code))
    ...
    cells.append(nbf.v4.new_markdown_cell(
        "## Section 4: Time-Series Telemetry & Pre-Onset Trajectories\n"
        "\n"
        "Multi-channel continuous telemetry visualization for individual patient trajectories and event-aligned "
        "pre-onset physiological trends leading up to adverse events (Hypotension, Hypoxia, Tachycardia)."
    ))
    ...
    cells.append(nbf.v4.new_markdown_cell(
        "## Section 5: Adverse Event Profiling & Target Separability\n"
        "\n"
        "Target prevalence analysis across adverse event classes (Future Hypotension, Future Hypoxia, Future Tachycardia), "
        "co-occurrence heatmaps, feature separability violin plots, and risk empirical cumulative distribution function (ECDF) curves."
    ))
    ...
    cells.append(nbf.v4.new_code_cell(fig12_code))
    ...
    cells.append(nbf.v4.new_code_cell(fig13_code))
    ...
    cells.append(nbf.v4.new_code_cell(fig14_code))
    ...
    cells.append(nbf.v4.new_code_cell(fig15_code))
    
    nb.cells = cells
    
    output_path = "dataset_eda.ipynb"
    with open(output_path, "w", encoding="utf-8") as f:
        nbf.write(nb, f)
        
    print(f"Successfully generated {output_path} with all 15 figures and 5 sections.")
