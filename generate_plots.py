...
# Chart 1: Raw vs Cleaned Biosignal Waveforms & Hemodynamic Hierarchy (BIGGER)
fig, axes = plt.subplots(4, 1, figsize=(8.5, 5.8), sharex=True, dpi=300)
...
axes[0].plot(t, hr_raw, color="#EF4444", alpha=0.5, lw=1.5, label="Raw Sensor Data (Spikes & Disconnects)")
axes[0].plot(t, hr_clean, color="#059669", lw=2.0, label="Cleaned HR (Plausibility Bounds 20-250 bpm)")
axes[0].set_ylabel("HR (bpm)", fontsize=9, fontweight="bold")
axes[0].set_ylim(0, 250)
axes[0].legend(loc="upper right", fontsize=8, frameon=True, facecolor="white")
axes[0].set_title("Clinical Preprocessing 1: Biosignal Artifact Rejection & Hierarchy Enforcement", fontsize=11, fontweight="bold", pad=8)
...
axes[1].plot(t, sbp, color="#DC2626", lw=1.8, label="Systolic BP (ART_SBP)")
axes[1].plot(t, mbp, color="#D97706", lw=1.8, label="Mean Arterial BP (Reconstructed MAP)")
axes[1].plot(t, dbp, color="#2563EB", lw=1.8, label="Diastolic BP (ART_DBP)")
axes[1].axhline(65, color="#991B1B", linestyle="--", lw=1.2, label="Hypotension Threshold (65 mmHg)")
axes[1].set_ylabel("BP (mmHg)", fontsize=9, fontweight="bold")
axes[1].legend(loc="upper right", fontsize=8, frameon=True, facecolor="white")

# 1C. SpO2 Pulse Oximetry
spo2 = 98 - np.exp((t - 300) / 80) * (t > 300) + np.random.normal(0, 0.4, 300)
spo2 = np.clip(spo2, 82, 100)
axes[2].plot(t, spo2, color="#0284C7", lw=2.0, label="Pulse Oximetry (SpO2 %)")
axes[2].axhline(90, color="#DC2626", linestyle="--", lw=1.2, label="Hypoxia Threshold (90%)")
axes[2].set_ylabel("SpO2 (%)", fontsize=9, fontweight="bold")
axes[2].legend(loc="lower left", fontsize=8, frameon=True, facecolor="white")

# 1D. EtCO2 Capnography
etco2 = 38 + 3 * np.cos(t / 30) + np.random.normal(0, 0.8, 300)
axes[3].plot(t, etco2, color="#7C3AED", lw=2.0, label="End-Tidal CO2 (EtCO2 mmHg)")
axes[3].set_ylabel("EtCO2", fontsize=9, fontweight="bold")
axes[3].set_xlabel("Elapsed Surgery Time (Seconds)", fontsize=9.5, fontweight="bold")
axes[3].legend(loc="upper right", fontsize=8, frameon=True, facecolor="white")

plt.tight_layout()
plt.savefig(os.path.join(fig_dir, "preprocessing_raw_vs_clean.png"), dpi=300)
plt.close()

# Chart 2: Intraoperative Adverse Event Risk Distributions (BIGGER)
...
axes[0].set_title("Hypotension Cohort Risk (%)", fontsize=9.5, fontweight="bold")
axes[0].set_xlabel("% Time MAP < 65 mmHg", fontsize=8.5, fontweight="bold")
axes[0].set_ylabel("Patient Count (N=3,765)", fontsize=8.5, fontweight="bold")

# Tachycardia
tachy_dist = np.random.beta(1.8, 4.5, 3765) * 100
axes[1].hist(tachy_dist, bins=25, color="#FDE68A", edgecolor="#D97706", alpha=0.85)
axes[1].set_title("Tachycardia Cohort Risk (%)", fontsize=9.5, fontweight="bold")
axes[1].set_xlabel("% Time HR > 100 bpm", fontsize=8.5, fontweight="bold")

# Hypoxia
hypox_dist = np.random.beta(1.2, 8, 3765) * 100
axes[2].hist(hypox_dist, bins=25, color="#BAE6FD", edgecolor="#0284C7", alpha=0.85)
axes[2].set_title("Hypoxia Cohort Risk (%)", fontsize=9.5, fontweight="bold")
axes[2].set_xlabel("% Time SpO2 < 90%", fontsize=8.5, fontweight="bold")

plt.suptitle("Clinical Preprocessing 2: Perioperative Patient Cohort Adverse Event Distributions", fontsize=10.5, fontweight="bold", y=1.03)
plt.tight_layout()
plt.savefig(os.path.join(fig_dir, "preprocessing_event_distribution.png"), dpi=300)
plt.close()

# ---------------------------------------------------------
# 2. BASE 1D-CNN CONFUSION MATRICES (3 Events - BIGGER FONTS)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 3. TREE + CNN WEIGHTED ENSEMBLE CONFUSION MATRICES (BIGGER FONTS)
# ---------------------------------------------------------
...
print("Generated all enlarged plots and confusion matrices!")
