...
export default function UserPage() {
  ...
  // Gated Auth: If not logged in, prompt for login
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-slate-100">
        <div className="w-full max-w-md bg-slate-900 border border-sky-950 rounded-2xl p-8 shadow-2xl text-center space-y-4">
          <div className="inline-flex p-3 bg-sky-950/80 rounded-xl border border-sky-800 text-sky-300">
            <User className="w-8 h-8" />
          </div>
          <h1 className="text-xl font-bold text-slate-100 uppercase">Staff Login Required</h1>
          <p className="text-xs text-slate-400">
            Please sign in with your staff user credentials to view `/user`.
          </p>
